...
def isotropic(double[:,:,::1] arr, int it, double dt, double Dx, double Dy, double Dz):
    # Variables:
    cdef int  i
    ...
    # Arrays:
    cdef double[:,:,::1] out = np.zeros((nx,ny,nz), dtype='float64')
    cdef double[:,:,::1] chg = np.zeros((nx,ny,nz), dtype='float64')
    out[:,:,:] = arr[:,:,:]
    # ==========================================================
    with nogil:
      for i in range(it):
