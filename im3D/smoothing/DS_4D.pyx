...
def isotropic(double[:,:,:,:] in_arr, int iters, 
    double dt, double D0, double D1, double D2, double D3, int bc_type):
    # ==========================================================
    # Variables:
    # a# is an axis
    ...
    # Arrays:
    cdef double[:,:,:,:] out   = np.zeros((n0,n1,n2,n3), dtype=np.float64)
    cdef double[:,:,:,:] dA_dt = np.zeros((n0,n1,n2,n3), dtype=np.float64)
    # Fill in values for out; central values with input array
    # and perimeter values using BCs:
    out[1:-1, 1:-1, 1:-1, 1:-1] = in_arr.copy()
    out = mirror_BC(out, 1)
    # ==========================================================
    with nogil:
        for i in range(iters):
                ...
                for a1 in range(1, n1-1):
                    for a2 in range(1, n2-1):
                        for a3 in range(1, n3-1):
                            g0 = (+1.0*out[a0-1, a1, a2, a3] \
                                  -2.0*out[a0+0, a1, a2, a3] \
                                  +1.0*out[a0+1, a1, a2, a3])/1.0
                            g1 = (+1.0*out[a0, a1-1, a2, a3] \
                                  -2.0*out[a0, a1+0, a2, a3] \
                                  +1.0*out[a0, a1+1, a2, a3])/1.0
                            g2 = (+1.0*out[a0, a1, a2-1, a3] \
                                  -2.0*out[a0, a1, a2+0, a3] \
                                  +1.0*out[a0, a1, a2+1, a3])/1.0
                            g3 = (+1.0*out[a0, a1, a2, a3-1] \
                                  -2.0*out[a0, a1, a2, a3+0] \
                                  +1.0*out[a0, a1, a2, a3+1])/1.0
                            #
            ...
            for a0 in prange(1, n0-1):
                for a1 in range(1, n1-1):
                    for a2 in range(1, n2-1):
                        for a3 in range(1, n3-1):
                            out[a0,a1,a2,a3] = out[a0,a1,a2,a3] + dt*dA_dt[a0,a1,a2,a3]
                        # end a3 for loop
                    # end a2 for loop
                # end a1 for loop
            # end a0 for loop
    ...
    return np.asarray(out[1:-1, 1:-1, 1:-1, 1:-1])

# ==============================================================
cdef inline double[:,:,:,:] mirror_BC(double[:,:,:,:] arr, ssize_t bdry) nogil:
    # ==========================================================
    # Variables:
    cdef int  i
    ...
    return arr
# ==============================================================
cdef inline double[:,:,:,:] fixed_BC(double[:,:,:,:] arr) nogil:
    # ==========================================================
    # Variables:
    cdef int  i
    cdef ssize_t  a0, n0=arr.shape[0]
