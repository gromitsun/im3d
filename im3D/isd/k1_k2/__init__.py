from .base_classes import k1k2_isd
...
class isd(k1k2_isd):
    ...
    def __init__(self, phi, k1, k2, P=None, nbins=200, bin_min=-0.1, bin_max=+0.1, eps=1.0):
        """
        add some info here about what is done in __init__
        """
        import numpy as np
        from .global_k1k2_isd import calculate_isd
        # 
        ...
        # 
        # Calculate the ISD:
        self.data = calculate_isd(phi, k1, k2, P, nbins, bin_min, bin_max, eps)
        # 
...
class local_isd(k1k2_isd):
    
    def __init__(self, phi, k1, k2, P=None, nbins=200, bin_min=-0.1, bin_max=+0.1, eps=1.0, k1_val=-0.05, k2_val=+0.05, dk=0.01, local_radius=5.0):
        """
        add some info here about what is done in __init__
        """
        import numpy as np
        from .local_k1k2_isd import calculate_isd
        # 
        # Create an array of ones for P if nothing is provided:
        if P==None:
            P = np.ones_like(phi)
        ...
        # 
        # Calculate the ISD:
        self.data = calculate_isd(phi, k1, k2, P, bin_min, bin_max, nbins, eps, local_radius, k1_val, k2_val, dk)
        # 
        # Save misc values for later use:
        self.k1_min = bin_min
        self.k1_max = bin_max
