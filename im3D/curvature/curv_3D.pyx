...
def H_32(float[:,:,::1] arr):
    ...
    cdef double  grad
    # Arrays:
    cdef float[:,:,::1] MC = np.zeros((nX,nY,nZ), dtype=np.float32)
    # Calculate curvature
    with nogil:
...
def H_64(double[:,:,::1] arr):
    ...
    cdef double  grad
    # Arrays:
    cdef double[:,:,::1] MC = np.zeros((nX,nY,nZ), dtype=np.float64)
    # Calculate curvature
    with nogil:
      for x in prange(2, nX-2):
        for y in range(2, nY-2):
...
#=== gaussian curvature; 32-bit=================================================
def K_32(float[:,:,::1] arr):
    # Variables:
    cdef ssize_t i
    cdef ssize_t x, nX=arr.shape[0]
    cdef ssize_t y, nY=arr.shape[1]
    ...
    cdef double  grad
    # Arrays:
    cdef float[:,:,::1] GC = np.zeros((nX,nY,nZ), dtype=np.float32)
    # Calculate curvature
    with nogil:
      for x in prange(2, nX-2):
        for y in range(2, nY-2):
    ...
    # Return curvature array
    return np.asarray(GC, dtype=np.float32)

#=== gaussian curvature; 64-bit=================================================
def K_64(double[:,:,::1] arr):
    # Variables:
    cdef ssize_t i
    cdef ssize_t x, nX=arr.shape[0]
    cdef ssize_t y, nY=arr.shape[1]
    ...
    cdef double  grad
    # Arrays:
    cdef double[:,:,::1] GC = np.zeros((nX,nY,nZ), dtype=np.float64)
    # Calculate curvature
    with nogil:
      for x in prange(2, nX-2):
        for y in range(2, nY-2):
      ...
      for x in range(nX):
          for y in range(nY):
              ...
              GC[x,y,nZ-1] = GC[x,y,nZ-3]
    # Return curvature array
    return np.asarray(GC, dtype=np.float64)

#=== 1st principal curvature; 32-bit ===========================================
def k1_32(float[:,:,::1] arr):
    # Variables:
    cdef ssize_t i
    cdef ssize_t x, nX=arr.shape[0]
    cdef ssize_t y, nY=arr.shape[1]
    ...
    # Arrays:
    cdef float[:,:,::1] K1 = np.zeros((nX,nY,nZ), dtype=np.float32)
    # Calculate curvature
    with nogil:
      for x in prange(2, nX-2):
        for y in range(2, nY-2):
    ...
    # Return curvature array
    return np.asarray(K1, dtype=np.float32)

#=== 1st principal curvature; 64-bit ===========================================
def k1_64(double[:,:,::1] arr):
    # Variables:
    cdef ssize_t i
    cdef ssize_t x, nX=arr.shape[0]
    cdef ssize_t y, nY=arr.shape[1]
    ...
    cdef double  grad, H, K
    # Arrays:
    cdef double[:,:,::1] K1 = np.zeros((nX,nY,nZ), dtype=np.float64)
    # Calculate curvature
    with nogil:
      for x in prange(2, nX-2):
        for y in range(2, nY-2):
...
def k2_32(float[:,:,::1] arr):
    ...
    cdef double  grad, H, K
    # Arrays:
    cdef float[:,:,::1] K2 = np.zeros((nX,nY,nZ), dtype=np.float32)
    # Calculate curvature
    with nogil:
      for x in prange(2, nX-2):
        for y in range(2, nY-2):
...
def k2_64(double[:,:,::1] arr):
    ...
    cdef double  grad, H, K
    # Arrays:
    cdef double[:,:,::1] K2 = np.zeros((nX,nY,nZ), dtype=np.float64)
    # Calculate curvature
    with nogil:
      for x in prange(2, nX-2):
        for y in range(2, nY-2):
      ...
      for x in range(nX):
          for y in range(nY):
              ...
              K2[x,y,nZ-1] = K2[x,y,nZ-3]
    # Return curvature array
    return np.asarray(K2, dtype=np.float64)
